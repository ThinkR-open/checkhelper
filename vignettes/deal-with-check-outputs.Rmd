...
Using `audit_tags()`, you can detect exported functions with missing or empty `@return` tag
...
- `audit_globals()` runs the checks and extracts no visible global variables and missing documented functions
- `fix_globals()` proposes a template to add missing global variables in a `globals.R` file (or writes it directly with `write = TRUE`).
...
```r
# Get globals
globals <- audit_globals(pkg_path)
globals
```
...
```r
# Or write the block directly to <pkg>/R/globals.R
fix_globals(pkg_path, write = TRUE)
```
