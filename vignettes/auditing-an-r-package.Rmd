...
fix_dataset_doc("my_data", pkg = pkg)  # one call per undocumented dataset
```
...
DESCRIPTION via the parser AST and reports every non-ASCII token
(string literal, comment, identifier). `fix_ascii()` rewrites them in
place — string literals become `\uXXXX` escapes, comments and roxygen
get `Latin-ASCII` transliteration. **It dry-runs by default**:

```{r}
audit_ascii(pkg)

# Always preview the diff first.
