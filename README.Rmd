...
The recommended workflow runs `R CMD check` **once** and reuses the result across every audit that needs it. `create_example_pkg()` ships a fake package that trips most of the checks `checkhelper` covers — use it to feel the flow:
