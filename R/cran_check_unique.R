...
cran_check_unique <- function(check_dir, lib_dir, scratch, Ncpus) {
  ...
  # if (grepl("ubuntu|debian", utils::osVersion)) {
  ...
  if (any(ind <- (args == "-l"))) {
    Sys.setenv(
      "_R_CHECK_CRAN_INCOMING_SKIP_URL_CHECKS_IF_REMOTE_" = "true",
  ...
  check_env_common <-
    c(
      paste0("LANG=", Sys.getenv("_R_CHECK_LANG_", "en_US.UTF-8")),
      ## (allow checking with LANG different from en_US.UTF-8)
      "LC_COLLATE=C",
      "LANGUAGE=en@quot",
      sprintf(
        "_R_CHECK_CRAN_STATUS_SUMMARY_=%s",
  ...
  check_env <-
    list(
      c(
        ...
        sprintf(
          "_R_CHECK_CRAN_INCOMING_SKIP_VERSIONS_=%s",
          !run_CRAN_incoming_feasibility_checks
        ),
        sprintf(
          "_R_CHECK_CRAN_INCOMING_SKIP_DATES_=%s",
          !run_CRAN_incoming_feasibility_checks
        ),
...
aspell_env_var <- function() {
  tryCatch(
    ...
    error = function(e) "_R_CHECK_CRAN_INCOMING_USE_ASPELL_=false"
  )
}
